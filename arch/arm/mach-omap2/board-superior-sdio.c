/* arch/arm/mach-omap2/board-superior-sdio.c
 *
 * Based on mach-omap2/board-kona-sdio.c
 *
...
struct mmc_platform_data superior_emmc_data = {
	.built_in		= 1,
};

static struct omap2_hsmmc_info superior_mmc_info[] = {
	{
		.mmc		= 2,
		.nonremovable	= true,
		.external_ldo	= false,
...
static void __init superior_hsmmc_set_late_init(struct device *dev)
{
	...
	pdata->init = superior_hsmmc_late_init;
}

static int __init superior_hsmmc_init(struct omap2_hsmmc_info *controllers)
{
	...
	for (c = controllers; c->mmc; c++)
		superior_hsmmc_set_late_init(c->dev);

	return 0;
}

void __init omap4_superior_sdio_init(void)
{
	superior_hsmmc_init(superior_mmc_info);
}
